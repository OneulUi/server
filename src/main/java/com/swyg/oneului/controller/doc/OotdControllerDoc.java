...
public interface OotdControllerDoc {
    ...
    @GetMapping("/ootds")
    ResponseEntity<CommonApiResponse<List<OotdDTO>>> getAllOotd();
    ...
    ResponseEntity<CommonApiResponse<OotdDTO>> getOotdById(@PathVariable(name = "ootdId") Long ootdId);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "유효하지 않은 인자 값")})
    @DeleteMapping("/ootds")
    void deleteOotd(Authentication authentication,
                           @RequestPart(name = "ootd") OotdDTO ootdDTO);
    ...
    ResponseEntity<CommonApiResponse<List<OotdDTO>>> getAllBookMarkOotd(Authentication authentication);
    ...
    ResponseEntity<CommonApiResponse<List<OotdDTO>>> getAllLikeOotd(Authentication authentication);
